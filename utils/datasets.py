		...
		with open(path, 'r', encoding='utf8') as fp:
			for lidx, line in enumerate(fp):
				...
				if instance_match is None:
					raise ValueError(f"[Error] Line {lidx} of '{path}' does not follow the dataset specification format.")
				...
				if (type(instance_labels) is list) and (len(instance_inputs) != len(instance_labels)):
					raise ValueError(f"[Error] Line {lidx} of '{path}' has an unequal number of sequence items and labels.")
